...
perf_dt_10 <- ROCR::performance(pred, measure = 'tpr', x.measure = 'fpr')
plot(perf_dt_10)
...
```{r}
data.frame(predicted=probs, actual=credit_test$default) %>%
  ggplot(data=., aes(x=predicted.No)) +
...
# plot ROC curve
pred_1 <- prediction(probs_1[, 2], credit_test$default)
perf_dt_1 <- performance(pred_1, measure = 'tpr', x.measure = 'fpr')
plot(perf_dt_1)
performance(pred_1, 'auc')
```

credit_cost_pred <- predict(credit_cost, credit_test)

#```{r trying to use ROC with cost}
